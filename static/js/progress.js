    ...
    try {
        ...
        if (!data.results || data.results.length === 0) {
            list.innerHTML = `
                <div class="loading">
                    ...
                    <p>
                        Play your first memory game and your progress
                        will appear here.
                    </p>
                </div>
            `;
        ...
        const recentResults = data.results.map(result => `
            <article class="reminder-item">
                ...
                <div>
                    <span class="difficulty-label ${difficultyClass(result.difficulty)}">
        ...
        // GAME PERFORMANCE
        // -----------------------------
        const gamePerformance = data.game_stats.map(game => `
            <div class="progress-game-card">
                ...
                <div class="progress-bar">
                    <div
                        class="progress-fill"
                        style="width: ${Math.max(0, Math.min(100, game.accuracy))}%">
                    </div>
                </div>
        ...
        const difficultyPerformance =
            data.difficulty_stats.map(level => `
                <div class="difficulty-card">
                    ...
                    <div>
                        <span class="difficulty-label ${difficultyClass(level.difficulty)}">
                            ${difficultyLabel(level.difficulty)}
                        </span>

                        <p>
                            ${level.played} game${level.played === 1 ? "" : "s"}
                        </p>
                    </div>
